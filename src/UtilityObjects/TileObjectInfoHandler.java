...
public class TileObjectInfoHandler extends SubMenu{
	...
	public void manageInfo() {
		...
		if(myPlayer.getTargetX()==Integer.MIN_VALUE || myPlayer.getTargetY()==Integer.MIN_VALUE){
			...
			myTileObjectInfo2 = new Text(900, 440, "COLLECTIBLE:", 1.5, myFont);
			if(c instanceof CollectibleSkill){
				myTileObjectInfo1 = new Text(900, 470, "NEW SKILL: " +  ((CollectibleSkill) c).getSkill().toUpperCase(), 1.5, myFont);
			}
			else{
				myTileObjectInfo1 = new Text(900, 470, c.getAmount() + " " + c.getType().toUpperCase() + " ENERGY", 1.5, myFont);
				if(c.getType().equals("health")){
					myTileObjectInfo1 = new Text(900, 470, c.getAmount() + " " + c.getType().toUpperCase(), 1.5, myFont);
				}
			}
		} else if(myCells.get(xLoc).get(yLoc).getPlayer()!=null){
			Player p = myCells.get(xLoc).get(yLoc).getPlayer();
			myTileObjectInfo5 = new Text(900, 350, "HP: " + p.getHealth() + "/" + p.getMaxHealth(), 1.5, myFont);
